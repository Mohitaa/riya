 ...
 */
App.controller('listConsultantController', function ($scope, $http, $route, $state, $cookies, $cookieStore, MY_CONSTANT, $timeout, ngDialog, responseCode) {
    'use strict';
    //$scope.showloader=true;
    $('#password_dialog').hide();
    $scope.delete_consultant_id = '';
    $scope.driver = {};
    $scope.currentPage = 0;
    $scope.flag = 0;
    $scope.displaymsg = '';
    ...
    //=====================================Data table view for all consultants===========================================
    $('#datatable2').on('click', '.edit', function (e) {
        $state.go('app.editConsultant', {consultant_id: e.currentTarget.id});
    });
    $('#datatable2').on('click', '.block', function (e) {
        $scope.blockunblockconsultant_popup(1, e.currentTarget.id);
    });
    $('#datatable2').on('click', '.unblock', function (e) {
        $scope.blockunblockconsultant_popup(0, e.currentTarget.id);
    });
    $('#datatable2').on('click', '.delete', function (e) {
        $scope.deleteconsultant_popup(e.currentTarget.id);
    });
    ...
    $('#datatable2').on('click', '.ViewDetails', function (e) {
        ...
        $state.go('app.viewConsultantDetails',{consultant_id: e.currentTarget.id});
    });
    /*--------------------------------------------------------------------------
    ...
    $scope.loadData = function () {
        ...
        $scope.dtInstance = $('#datatable2').dataTable({
            ...
            "bServerSide": true,
            sAjaxSource: MY_CONSTANT.url + "/get_all_consultant_data?access_token=" + $cookieStore.get('obj').accesstoken,
            oLanguage: {
...
//loops through rows
    ...
    /*--------------------------------------------------------------------------
     * ------------- funtion to open modal for delete consultant ----------------
     --------------------------------------------------------------------------*/
    $scope.deleteconsultant_popup = function (delete_consultant_id) {
        console.log("gdagdahg", delete_consultant_id);
        $scope.delete_consultant_id = delete_consultant_id;
        ngDialog.open({
            template: 'delete_consultant_modalDialog',
    ...
    /*--------------------------------------------------------------------------
     * -------------------------funtion to delete consultant --------------------------
     --------------------------------------------------------------------------*/
    $scope.delete_consultant = function (delete_consultant_id) {
        ngDialog.close();
        $.post(MY_CONSTANT.url + '/delete_user', {
            access_token: $cookieStore.get('obj').accesstoken,
            user_id: delete_consultant_id
        }, function (data) {
            
            $scope.displaymsg = '';
            if (data.status == responseCode.SUCCESS) {
                $scope.displaymsg = "consultant is deleted successfully.";
            }
...
//    /*--------------------------------------------------------------------------
//     * --------- funtion to open dialog for block or unblock consultant ------------
//     --------------------------------------------------------------------------*/
    $scope.blockunblockconsultant_popup = function (is_blocked, user_id) {
        $scope.blockunblockmsg = '';
        $scope.blockunblockid = user_id;
        $scope.is_blocked = is_blocked;
        if (is_blocked == 1) {
            $scope.blockunblockmsg = "Are you sure you want to block this consultant?";
        } else {
            $scope.blockunblockmsg = "Are you sure you want to unblock this consultant?";
        }
        ngDialog.open({
            template: 'block_unblock_consultant_modalDialog',
            className: 'ngdialog-theme-default',
            showClose: false,
            scope: $scope,
            closeByEscape: false,
    ...
    /*--------------------------------------------------------------------------
     * ------------------ funtion to block or unblock consultant -------------------
     --------------------------------------------------------------------------*/
    $scope.do_block_unblock_consultant = function (is_blocked, blockunblockid) {
        $.post(MY_CONSTANT.url + '/block_unblock_user',
            {
                ...
                if (data.status == responseCode.SUCCESS) {
                    if (is_blocked == 1) {
                        $scope.displaymsg = "consultants is blocked successfully.";
                    } else {
                        $scope.displaymsg = "consultants is unblocked successfully.";
                    }
                }
                else if (data.status == responseCode.INVALID_ACCESS_TOKEN) {
                    $state.go('page.login');
